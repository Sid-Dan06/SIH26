...
class QuizScreen extends StatefulWidget {
  ...
  const QuizScreen({super.key, this.targetSkill});
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  Future<void> loadBackendQuestions() async {
    try {
      final qList = await ApiService.fetchQuestions(skill: widget.targetSkill);
      if (mounted) {
        setState(() {
          liveQuestions = qList;
          loading = false;
    ...
    if (currentQuestionIndex + 1 <
      ...
      try {
        ...
        if (mounted) {
          ...
          // 🌟 Navigate straight to AppShell Dashboard after Quiz completion!
